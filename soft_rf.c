...
bit_time init_timings_(uint32_t speed, uint32_t timer_freq)
{
	...
	bt.timer_ticks_per_bit_ = round(bt.bit_time_ / \
		bt.one_timer_tick_time_);
	// допуск на длину бита +- 1/50 длительности бита
	bt.delta_timer_ticks_per_bit_ = bt.timer_ticks_per_bit_ / 50;
	bt.timer_ticks_per_bit_min_ = bt.timer_ticks_per_bit_ - \
		bt.delta_timer_ticks_per_bit_;
	bt.timer_ticks_per_bit_max_ = bt.timer_ticks_per_bit_ + \
		bt.delta_timer_ticks_per_bit_;
	bt.start_bit_ticks_ = 3 * bt.timer_ticks_per_bit_;
...
uint8_t Add_signal_to_sequence(bit_time* bt, uint16_t* buffer, timer_receive_sequence *sequence, data_full_msg * data)
{
	...
	sequence->receive_sequence_[sequence->sequence_iterator_] = buffer[0];
	++sequence->sequence_iterator_;
	sequence->receive_sequence_[sequence->sequence_iterator_] = buffer[1];
	++sequence->sequence_iterator_;
...
uint8_t Convert_sequence_find_data_length(bit_time* bt, timer_receive_sequence* tim_seq, data_full_msg* data )
{
	...
	static uint16_t buffer_ = { 0, };  // буфер для заполнения 6 битными значениями
	static uint8_t buffer_iterator_ = 0;
	static uint8_t word_buffer_ = 0; // заполняется по 4 бита, для учета нужен итератор
	static uint8_t halfword_iterator_ = 0;
	...
	for (uint8_t i = start_; i < tim_seq->sequence_iterator_; i = i + 2) {//TODO: что если sequence_iterator нечетный
		...
		count_1 = tim_seq->receive_sequence_[i] / bt->timer_ticks_per_bit_;  // TODO: 16 BIT TO 8 BIT
		if (tim_seq->receive_sequence_[i] % bt->timer_ticks_per_bit_ > \
			bt->timer_ticks_per_bit_min_ * count_1) ++count_1; // проверяю остаток
		count_0 = tim_seq->receive_sequence_[i + 1] / bt->timer_ticks_per_bit_;
		if (tim_seq->receive_sequence_[i + 1] % bt->timer_ticks_per_bit_ > \
			bt->timer_ticks_per_bit_min_ * count_0) ++count_0;
