 ...
 * 
 *  План:
 * поиск вступление "забор" 101010...101010 (ищет 2 таймер и функция on_timer_count_interrupt)
 * (при срабатывании прерывания по количеству обработанных сообщений - во 2 таймере меняю буфер у первого, что бы он продолжил сохранение данных)
 * поиск начала сигнала - поиск во входящей последовательности двух стартовых последовательностей 0х38 ( в функции on_timer_count_interrupt )
 * если начало найдено расшифровка с начала ( пока расшифровка всей последовательности)
 * получение длины сообщения (в функции on_timer_count_interrupt )
 * прием начало
